...
class SumDistro extends Rule {
	evalRoll = (dice) => {
		...
		if (this.freq(dice).some((c) => c >= this.count)) {
		...
		// 	this.freq(dice).some((c) => c >= this.count) ? this.sum(dice) :
		// 	0;
	};
}
...
class FullHouse extends Rule {
	evalRoll = (dice) => {
		...
		let freqAns = freqs.includes(3) && freqs.includes(2);
		if (freqAns) {
			return this.score;
		} else {
			return 0;
...
class LargeStraight extends Rule {
	evalRoll = (dice) => {
		...
		// 	d.size === 5 && (!d.has(1) || !d.has(6)) ? this.score :
		// 	0;
	};
}
